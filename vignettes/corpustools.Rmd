...
* adding, removing or mutating columns in the tc$tokens and tc$meta data.tables
* subsetting rows in the tc$tokens and tc$meta data.tables

While it is possible to directly work in the tc$tokens and tc$meta data.tables, it is recommended to use the special accessor functions. The tCorpus makes several assumptions about how the data is structured in order to work fast, and directly editing the tokens and meta data can thus break the tCorpus. Using the accessor functions should ensure that this doesn't happen.
...
To use a UDPipe model in create_tcorpus, you simply need to use the `udpipe_model` argument. The value for this  argument is the language/name of the model you want to use (if the name does not exist, you'll get a list of options). The first time using a language, R will automatically download the model. By default the udpipe model is stored in the current working directory, in a folder called udpipe_models, but you can set the path with the `udpipe_model_path` parameter.
...
We can use the aggregate method to aggregate hits. Here we use `wide = TRUE` to return results in wide format.
...
The tCorpus is mainly intended for managing, querying, annotating and browsing tokens in a tokenlist format, but there are some additional analysis techniques. Here we use tokens with basic preprocessing as an example, but note that these techniques can be applied for all features and annotations in a tCorpus. 
