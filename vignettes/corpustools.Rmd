...
The following code shows an example (not shown in this vignette, because it depends on quanteda). We first 
...
The tokens data now has an additional column "code" with the labels of the dictionary. For the LSD2015 dictionary these labels are "positive" and "negative", and the negated terms "neg_positive" and "neg_negative". If we represent these labels as a scale from -1 (negative) to 1 (positive), we can create a browser in which the sentiment scores of words are visualized with a color scale.

While it is possible to create the numeric scores for sentiment within the tCorpus, we'll show another option here. Instead of using the quanteda dictionary, we can use a data.frame as a dictionary as long as it has a column named "string" that contains the dictionary pattern. All other columns in the data.frame are then added to $tokens. With the `melt_quanteda_dict` function we can also convert a quanteda dictionary to this type of data.frame. The code is then as follows:
