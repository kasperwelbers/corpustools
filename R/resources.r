...
#'
#' @export
feature_from_resource <- function(tc, resource='jrc_names', new_feature=resource, feature='word', resource_path=NULL, collocation_labels=T, verbose=T, batchsize='default'){
...
#' local_path can also be tempdir(), in which case the resource is deleted when the R session closes.
#'
#' @param resource A character string indicating which resource to download
#' @param local_path The path (without the filename) where the resource is stored. If NULL, the file will be stored in the location of the tcorpus package. if tempdir(), the file is stored in a directory that is cleaned up when the R session ends.
...
#'
#' @export
download_resource <- function(resource=c('jrc_names'), local_path=NULL, force_download=F){
  resource = match.arg(resource)
...
#'
#' @export
load_resource <- function(resource=c('jrc_names'), local_path=NULL){
  resource = match.arg(resource)
  fname = make_filename(local_path, resource)
