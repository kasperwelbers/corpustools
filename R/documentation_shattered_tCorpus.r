...
#'
#' Shatter a tCorpus into one or more shards (i.e. subsets). Each subset is saved on disk instead of being kept in memory, and will only be loaded into memory when required.
...
#' Ideally, you want to study the whole vase, and let the tedious but necessary shattering and glueing to be done for you, behind the scenes.
...
#' Can I big data now?
#' \itemize{
#' \item This functionality is not competing with big data software such as Hadoop. The goal is not to facilitate cluster computing of shards. Still, if you have a decent computer and super-speed is not your goal, then the shattered_tCorpus approach should enable you to scale up pretty well.
#' }
