...
search_string <- function(fi, string, allow_multiword=T, allow_proximity=T, only_last_mword=T){
  ...
  if (nrow(proxi) > 0){
    ...
    hit_proxi_index = proximity_grepl(fi, proxi, proxi$window, only_last=only_last_mword)
  ...
  for(i in 1:length(mword_regex)){
    ...
    for(q in mword){
      if (q == mword[1]) {   ## if first word, search everything
        hit = fi$global_i[grepl(q, fi$feature, ignore.case=ign_case, perl=perl, useBytes=useBytes)]
        ...
        hit = fi_next$global_i[grepl(q, fi_next$feature, ignore.case=ign_case, perl=perl, useBytes=useBytes)]
      }
...
proximity_grepl <- function(fi, proxi, windows, only_last=T, ignore.case=T, perl=F, useBytes=T){
  hits = list()
  ...
  for(i in 1:length(pword_regex)){
    ...
    for(j in 1:length(q)){
        i = fi$global_i[grepl(q[j], fi$feature, ignore.case=ign_case, perl=perl, useBytes=useBytes)]
        if (length(i) > 0){
          if (only_last){
