...
tcorpus_compare <- function(tc_x, tc_y, feature, context_level=c('document','sentence'), smooth=1, min.over=NULL, min.chi=NULL, x_is_subset=F) {
  dtm_compare(tc_x$dtm(feature, context_level, with_labels = F), tc_y$dtm(feature, context_level, with_labels = F), smooth=smooth, min.over=min.over, min.chi=min.chi, x_is_subset=x_is_subset)
}
